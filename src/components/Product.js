...
function Product() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Seller/AllProduct', {
          headers: { 'auth': token }
        });
  ...
  const handleDelete = async (productId) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const token = localStorage.getItem('Token');
        const response = await axios.delete(`https://ecommerce-platform-kfby.onrender.com/Seller/DeleteProduct/${productId}`, {
          headers: { 'auth': token }
        });
