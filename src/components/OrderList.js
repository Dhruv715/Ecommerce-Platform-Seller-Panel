...
function OrderList() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Seller/getSellerProductsAndOrders', {
          headers: {
            'auth': token
          }
        });
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      const token = localStorage.getItem('Token');
      const response = await axios.patch('https://ecommerce-platform-kfby.onrender.com/Seller/UpdateStatus', {
        orderId,
        newStatus,
      }, {
        headers: {
