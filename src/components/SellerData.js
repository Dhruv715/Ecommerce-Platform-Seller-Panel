...
const SellerData = () => {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      try {
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Seller/SellerData', {
          headers: {
            'auth': token,
          },
        });
        ...
        sellerInfo ? (
          <Paper elevation={3} sx={{ p: 2 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <img
                 src={`https://ecommerce-platform-kfby.onrender.com/images/${sellerInfo.brandLogo}`}
                alt="Profile"
