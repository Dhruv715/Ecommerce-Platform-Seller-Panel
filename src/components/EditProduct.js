...
function EditProduct({ product, onClose, onSave }) {
  ...
  const handleAddImage = async (e) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post('https://ecommerce-platform-kfby.onrender.com/Seller/UploadImage', formData, {
          headers: {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.patch(`https://ecommerce-platform-kfby.onrender.com/Seller/UpdateProduct/${product._id}`, editedProduct, {
        headers: { 'auth': token }
      });
          ...
          {editedProduct.image.map((img, index) => (
            <Grid item key={index} xs={6} sm={4} md={3}>
              <div style={{ position: 'relative' }}>
                <img src={`https://ecommerce-platform-kfby.onrender.com/images/${img}`} alt={`product ${index}`} style={{ width: '100%' }} />
                <IconButton
